...
export default function PublicMenu({ onBack }: { onBack: () => void }) {
  ...
  const [customerName, setCustomerName] = usePersistentState<string>("batidinha:customer:name", "");
  const [customerPhone, setCustomerPhone] = usePersistentState<string>("batidinha:customer:phone", "");
  const [customerAddress, setCustomerAddress] = usePersistentState<string>("batidinha:customer:address", "");
  ...
  const customDrinkTotal = flavorPrice + crustPrice + extrasPrice;
  ...
  function addCustomDrink() {
    ...
    setCart((current) => [...current, { id: crypto.randomUUID(), name: `Batidinha ${size.name}`, detail: `${chosenFlavors.map((item) => item.name).join(" + ")} • ${crust?.name}${selectedExtras.length ? ` • ${selectedExtras.map((item) => item.name).join(", ")}` : ""}`, price: customDrinkTotal, quantity: 1, source: { kind: "pizza", sizeId: size.id, flavorIds, crustId: crust?.id, extraIds } }]);
