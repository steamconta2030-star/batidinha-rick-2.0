...
import { ArrowLeft, Check, CheckCircle2, CupSoda, Layers3, MapPin, MessageCircle, Minus, Plus, Search, ShoppingBag, Sparkles, Trash2, X } from "lucide-react";
...
export default function PublicMenu({ onBack }: { onBack: () => void }) {
  ...
  const activeSizes = sizes.filter((item) => item.active);
  const maxFlavorsAvailable = Math.max(1, ...activeSizes.map((item) => item.maxFlavors));
  ...
  return <div className="public-menu">
    <header className="public-header"><button className="back-admin" onClick={onBack}><ArrowLeft size={17} /> Administração</button><div className="public-brand"><span><CupSoda size={22} /></span><div><strong>Batidinha do Rick</strong><small>{online ? `Cardápio online • ${storeOpen ? "Loja aberta" : "Loja fechada"}${minimumOrder ? ` • Mín. ${money.format(minimumOrder)}` : ""}` : "Gelada, cremosa e feita para você."}</small></div></div><button className="cart-button" onClick={() => setCartOpen(true)}><ShoppingBag size={19} /><span>{cartCount}</span><b>{money.format(cartTotal)}</b></button></header>
    ...
    <section className="benefit-strip" aria-label="Diferenciais da Batidinha do Rick"><article><Sparkles size={21} /><div><strong>Feita na hora</strong><span>Cremosa e bem gelada</span></div></article><article><Layers3 size={21} /><div><strong>Até {maxFlavorsAvailable} sabores</strong><span>{activeSizes.length} opções de tamanho</span></div></article><article><CupSoda size={21} /><div><strong>Do seu jeito</strong><span>Preparo e adicionais</span></div></article><article><MapPin size={21} /><div><strong>Entrega ou retirada</strong><span>Escolha ao finalizar</span></div></article></section>
