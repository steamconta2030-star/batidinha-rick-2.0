...
export default function PublicMenu({ onBack }: { onBack: () => void }) {
  ...
  async function finishOrder(event: FormEvent<HTMLFormElement>) {
    ...
    const parsed = checkoutSchema.safeParse({
      ...
      address: String(data.get("address") ?? ""), paymentMethod: String(data.get("paymentMethod") ?? "pix"),
  ...
  function confirmOnWhatsapp(order: Order) {
    ...
    const itemLines = order.items.map((item) => `${item.quantity}x ${item.name}${item.detail ? ` — ${item.detail}` : ""}`).join("\n");
  ...
  return <div className="public-menu">
    <header className="public-header"><button className="back-admin" onClick={onBack}><ArrowLeft size={17} /> Administração</button><div className="public-brand"><span><Flame size={22} /></span><div><strong>Batidinha do Rick</strong><small>{online ? `Cardápio online • ${storeOpen ? "Loja aberta" : "Loja fechada"}${minimumOrder ? ` • Mín. ${money.format(minimumOrder)}` : ""}` : "Gelada, cremosa e feita para você."}</small></div></div><button className="cart-button" onClick={() => setCartOpen(true)}><ShoppingBag size={19} /><span>{cartCount}</span><b>{money.format(cartTotal)}</b></button></header>
    <section className="public-hero"><div><small>🥤 BATIDINHA GELADA • ENTREGA RÁPIDA</small><h1>Sua batidinha, do seu jeito.</h1><p>Escolha seus sabores favoritos e receba geladinha em casa.</p><button onClick={() => setBuilderOpen(true)}>Personalizar minha batidinha <Plus size={18} /></button></div></section>
