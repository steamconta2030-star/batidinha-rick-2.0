...
export default function Operations({ view }: { view: "dashboard" | "kitchen" | "cash" }) {
  ...
  return <section className="content dashboard-page"><div className="title-row"><div><p className="eyebrow">VISÃO GERAL</p><h1>Olá, Batidinha!</h1><p>Acompanhe os principais números da operação de hoje.</p></div><span className="dashboard-date">{new Date().toLocaleDateString("pt-BR", { weekday: "long", day: "2-digit", month: "long" })}</span></div><div className="dashboard-metrics"><article><span><ShoppingBag /></span><div>Pedidos hoje<strong>{todayOrders.length}</strong><small>{todayOrders.filter((item) => !["delivered"].includes(item.status)).length} ativos</small></div></article><article><span><Banknote /></span><div>Valor dos pedidos<strong>{money.format(orderedRevenue)}</strong><small>Atualiza ao comprar</small></div></article><article><span><Clock3 /></span><div>Ticket médio<strong>{money.format(todayOrders.length ? orderedRevenue / todayOrders.length : 0)}</strong><small>{todayOrders.length} pedidos realizados</small></div></article><article><span><ChefHat /></span><div>Na cozinha<strong>{orders.filter((item) => ["confirmed", "preparing"].includes(item.status)).length}</strong><small>Aguardando ou preparando</small></div></article></div><div className="dashboard-grid"><article className="revenue-panel"><header><div><h2>Pedidos por pagamento</h2><p>Distribuição de todos os pedidos de hoje.</p></div><strong>{money.format(orderedRevenue)}</strong></header>{(["pix", "cash", "card"] as const).map((method) => { const value = todayOrders.filter((order) => order.paymentMethod === method).reduce((sum, order) => sum + order.total, 0); return <div className="revenue-bar" key={method}><span>{method === "pix" ? "PIX" : method === "cash" ? "Dinheiro" : "Cartão"}</span><i><b style={{ width: `${(value / maxRevenue) * 100}%` }} /></i><strong>{money.format(value)}</strong></div> })}</article><article className="recent-panel"><header><h2>Pedidos recentes</h2><span>{orders.length} no histórico</span></header>{orders.slice(0, 5).map((order) => <div key={order.id}><span>#{String(order.number).padStart(3, "0")}</span><div><strong>{order.customerName}</strong><small>{order.items.length} itens • {order.deliveryType === "delivery" ? "Entrega" : "Retirada"}</small></div><b>{money.format(order.total)}</b></div>)}{orders.length === 0 && <p>Nenhum pedido ainda.</p>}</article></div></section>;
}
